...
function prefillProfileForm() {
  ...
  if (myProfile.avatar) {
    document.querySelectorAll('.avatar-option').forEach(a => {
      a.classList.toggle('selected', a.dataset.avatar === myProfile.avatar);
    });
...
function renderMyProfile() {
  ...
  if (!currentUser) {
    container.innerHTML = `<div style="text-align:center;padding:80px 20px">
      ...
      <h3 style="font-family:'Syne',sans-serif;font-size:24px;font-weight:800;margin-bottom:10px">Not signed in</h3>
  ...
  if (!myProfile) {
    container.innerHTML = `<div style="text-align:center;padding:80px 20px">
      <div style="font-size:48px;margin-bottom:16px">🪴</div>
      <h3 style="font-family:'Syne',sans-serif;font-size:24px;font-weight:800;margin-bottom:10px">No profile yet</h3>
      <p style="color:var(--muted);margin-bottom:24px">Create your profile to be discovered.</p>
      <button class="btn-primary" onclick="showPage('create-profile')">Create profile</button>
    </div>`;
    return;
  }
  ...
  container.innerHTML = `
    <div class="my-profile-card">
      ...
      <div class="tag-group">
        ${(myProfile.have || []).map(s => `<span class="tag tag-have">${s}</span>`).join('') || '<span style="color:var(--muted);font-size:14px">None added</span>'}
      </div>
      <div class="section-label">Skills I want to learn</div>
      <div class="tag-group">
        ${(myProfile.want || []).map(s => `<span class="tag tag-want">${s}</span>`).join('') || '<span style="color:var(--muted);font-size:14px">None added</span>'}
      </div>
    ...
    <div style="background:var(--card-bg);border:1px solid var(--border);border-radius:16px;padding:24px;margin-bottom:20px;display:grid;grid-template-columns:1fr 1fr;gap:20px">
      <div>
        <div style="font-family:'Syne',sans-serif;font-weight:800;font-size:28px;color:var(--accent)">${connected}</div>
        <div style="color:var(--muted);font-size:13px;margin-top:4px">Connections</div>
      </div>
      <div>
        <div style="font-family:'Syne',sans-serif;font-weight:800;font-size:28px;color:var(--accent3)">${(myProfile.have || []).length + (myProfile.want || []).length}</div>
        <div style="color:var(--muted);font-size:13px;margin-top:4px">Skills listed</div>
      </div>
    </div>
    <div>
...
function addTag(containerId, inputId, arr, cls) {
  ...
  input.addEventListener('keydown', e => {
    ...
    if (e.key === 'Backspace' && !input.value && arr.length) {
      arr.pop();
      renderTags(containerId, inputId, arr, cls);
    }
  });
}
...
function renderTags(containerId, inputId, arr, cls) {
  ...
  container.innerHTML = arr.map((t, i) => `
    <span class="skill-tag ${cls}">${t}
      <span class="skill-tag-remove" onclick="removeTag('${containerId}','${inputId}',${i},'${cls}')">×</span>
    </span>`).join('');
  container.appendChild(input);
}

function removeTag(containerId, inputId, idx, cls) {
  if (containerId === 'have-tags-container') {
    haveTags.splice(idx, 1);
    renderTags(containerId, inputId, haveTags, cls);
  } else {
    wantTags.splice(idx, 1);
    renderTags(containerId, inputId, wantTags, cls);
  }
}
